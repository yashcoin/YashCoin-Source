YashCoin-qt: Qt4 GUI for YashCoin
===============================
...
Alternatively, install Qt Creator and open the `YashCoin-qt.pro` file.

An executable named `YashCoin-qt` will be built.
...
A warning for people using the *static binary* version of YashCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of YashCoin is linked against libdb5.3.
...
installed causes YashCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
