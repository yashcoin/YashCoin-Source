...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #YashCoinTEST2\r");
            Send(hSocket, "WHO #YashCoinTEST2\r");
        } else {
            // randomly join #YashCoin00-#YashCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #YashCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #YashCoin%02d\r", channel_number).c_str());
        }
